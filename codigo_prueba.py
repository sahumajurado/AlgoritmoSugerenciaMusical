...
def registrar_cancion_escuchada(id_usuario, id_cancion):
    fecha_actual = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    query = "INSERT INTO cancion_escuchada (fecha, id_cancion, id_usuario) VALUES (%s, %s, %s)"
    datos = (fecha_actual, id_cancion, id_usuario)
    
    try:
        cursor.execute(query, datos)
        conexion.commit()
        print("------------------------------------------------")
        print("Canción registrada como escuchada.")
        print("------------------------------------------------")
    except Exception as e:
        print("------------------------------------------------")
        print(f"Error al registrar la canción escuchada: {e}")
        print("------------------------------------------------")

def reproducir_musica(archivo_musica, id_usuario, id_cancion):
    # Intenta abrir el reproductor predeterminado con la canción actual
    webbrowser.open(archivo_musica)
    registrar_cancion_escuchada(id_usuario, id_cancion)
            ...
            while True:
                ...
                if cancion_id == 's':
                    ...
                    if any(cancion[0] == int(cancion_id) for cancion in favoritos):
                        ...
                        reproducir_musica(archivo_musica, resultado_usuario[0], cancion_id)
                    else:
