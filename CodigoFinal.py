...
def reproducir_musica(archivo_musica):
    ...
    if archivo_musica:
        ...
        print("No se pudo encontrar el archivo de música.")
        ...
        if favoritos:
            ...
            while True:
                ...
                if cancion_id == 's':
                    ...
                    if any(cancion[0] == int(cancion_id) for cancion in favoritos):
                        ...
                        archivo_musica_resultado = cursor.fetchone()
                        if archivo_musica_resultado:
                            archivo_musica = archivo_musica_resultado[0]
                            reproducir_musica(archivo_musica)
                        else:
                            print("No se pudo encontrar el archivo de música para la canción seleccionada.")
                    else:
                        print("No hay canciones en la lista de favoritos.")
        else:
            print("No hay canciones en la lista de favoritos.")
    else:
        print("Debe iniciar sesión para acceder a la lista de favoritos.")

def lista_recomendacion():
    if resultado_usuario:
        id_usuario = resultado_usuario[0]  # Obtener el ID del usuario
        query_recomendaciones = """
        SELECT c.id_cancion,lr.id_lista_recomendacion, c.nombre AS nombre_cancion, 
               a.nombre AS nombre_artista, al.nombre AS nombre_album, g.nombre AS nombre_genero
        FROM lista_recomendacion AS lr
        JOIN cancion AS c ON lr.id_cancion = c.id_cancion
        JOIN artista AS a ON c.id_artista = a.id_artista
        JOIN album AS al ON c.id_album = al.id_album
        JOIN generomusical AS g ON c.id_generomusical = g.id_generomusical
        WHERE lr.id_usuario = %s
        """
        cursor.execute(query_recomendaciones, (id_usuario,))
        ...
        if recomendaciones:
            ...
            while True:
                ...
                if cancion_id == 's':
                    ...
                    if any(cancion[0] == int(cancion_id) for cancion in recomendaciones):
                        query_reproducir = "SELECT archivo_musica FROM cancion WHERE id_cancion = %s"
                        datos_reproducir = (cancion_id,)
                        cursor.execute(query_reproducir, datos_reproducir)
                        archivo_musica_resultado = cursor.fetchone()
                        ...
                        print("El ID ingresado no corresponde a una canción en la lista de recomendaciones.")
        else:
            print("No hay canciones en la lista de recomendaciones.")
    else:
        print("Debe iniciar sesión para acceder a la lista de recomendaciones.")
...
def principal():
    if resultado_usuario:
        ...
        print("3. Escuchar lista de recomendación")
        print("4. Salir")
        print("------------------------------------------------")
